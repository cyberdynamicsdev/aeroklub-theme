...
@section('content')
    @php
        // --- Dane z opcji motywu (Strona główna) ---
        $heroImg = get_field('hero_obraz', 'option');
        $heroEyebrow = get_field('hero_eyebrow', 'option') ?: 'Sport lotniczy · klasa mikrolotowa';
        $heroTytul = get_field('hero_tytul', 'option') ?: 'Lataj, rywalizuj,<br>reprezentuj Polskę.';
        ...
        $heroPodpis = get_field('hero_podpis', 'option');
        ...
        $galeria = get_field('galeria', 'option') ?: [];
        $galeriaLink = get_field('galeria_link', 'option');

        $ctaEyebrow = get_field('cta_eyebrow', 'option') ?: 'Dołącz do sportu';
        $ctaTytul = get_field('cta_tytul', 'option') ?: 'Masz licencję i marzenie o zawodach? Zacznij od pierwszego startu.';
        $ctaTekst = get_field('cta_tekst', 'option') ?: 'Prowadzimy zawodników od pierwszych zawodów regionalnych aż po kadrę narodową i mistrzostwa świata FAI. Sprawdź, jak dołączyć do rywalizacji.';
        $ctaBtnLabel = get_field('cta_btn_label', 'option') ?: 'Dowiedz się jak zacząć';
        $ctaBtnLink = get_field('cta_btn_link', 'option');

        // --- Zapytania ---
        $zawodyQ = new WP_Query([
            'post_type' => 'zawody',
            'posts_per_page' => 3,
            'meta_key' => 'data_start',
            ...
            'order' => 'ASC',
            'meta_query' => [
                ['key' => 'status', 'value' => ['planowane', 'w_trakcie'], 'compare' => 'IN'],
            ],
        ]);

        $newsQ = new WP_Query(['post_type' => 'post', 'posts_per_page' => 3]);
        $kadraQ = new WP_Query(['post_type' => 'zawodnik', 'posts_per_page' => 6]);

        $btnUrl = fn ($link, $fallback = '#') => is_array($link) ? ($link['url'] ?? $fallback) : ($link ?: $fallback);
    @endphp

    {{-- ============ HERO ============ --}}
    <section class="relative bg-navy text-white overflow-hidden flex items-stretch" style="min-height:clamp(460px,62vh,640px);">
        @if ($heroImg)
            <img src="{{ $heroImg['url'] }}" alt="{{ $heroImg['alt'] ?: '' }}" class="absolute inset-0 w-full h-full object-cover" style="object-position:center 40%;" />
        @endif
        ...
        <div class="relative z-[2] w-full container-site flex flex-col justify-center" style="padding-block:clamp(56px,9vw,104px);">
            <div style="max-width:700px;">
                ...
                <h1 class="font-heading font-extrabold m-0 mb-[22px]" style="font-size:clamp(36px,5.6vw,62px);line-height:1.05;letter-spacing:-0.01em;">{!! $heroTytul !!}</h1>
                ...
                <div class="flex flex-wrap items-center gap-3.5">
                    <a href="{{ $btnUrl($btn1Link, '#zawody') }}" class="btn btn-gold">{{ $btn1Label }}</a>
                    <a href="{{ $btnUrl($btn2Link, '#start') }}" class="btn btn-outline-light">{{ $btn2Label }}</a>
                </div>
            </div>
        </div>

        @if ($heroPodpis)
            <div class="absolute bottom-3 z-[2]" style="right:clamp(20px,4vw,40px);font-size:11px;color:rgba(255,255,255,0.45);">{{ $heroPodpis }}</div>
        @endif
    ...
    @if ($newsQ->have_posts())
        ...
        @endphp
        <section id="aktualnosci" class="section-y bg-surface">
            <div class="container-site">
                <x-section-head
                    eyebrow="Z życia komisji"
                    title="Ostatnie aktualności"
                    :link="get_permalink(get_option('page_for_posts')) ?: home_url('/')"
                ...
                <div class="grid gap-[26px] items-start" style="grid-template-columns:repeat(auto-fit,minmax(320px,1fr));">
                    ...
                    <article class="card flex flex-col overflow-hidden">
                        ...
                        <div class="flex-1 flex flex-col" style="padding:26px 28px 28px;">
                            @php($fcat = get_the_category($featured->ID))
                            @if ($fcat)
                                <span class="uppercase font-bold text-gold-dark mb-3" style="font-size:11px;letter-spacing:0.1em;">{{ $fcat[0]->name }}</span>
                            @endif
                    ...
                    <div class="flex flex-col gap-[18px]">
                            ...
                            <article class="card flex overflow-hidden">
                                ...
                                <div class="flex-1" style="padding:18px 20px;">
                                    <div class="flex gap-3 items-center mb-2">
                                        @php($sc = get_the_category())
                                        @if ($sc)
                                            <span class="uppercase font-bold text-gold-dark" style="font-size:10.5px;letter-spacing:0.1em;">{{ $sc[0]->name }}</span>
                                        @endif
                        ...
                        @endforeach
                        @php(wp_reset_postdata())
                        <a href="{{ get_permalink(get_option('page_for_posts')) ?: home_url('/') }}" class="border border-line-2 text-center uppercase font-bold text-navy hover:bg-[#e9ecf2]" style="padding:16px;font-size:13px;letter-spacing:0.04em;">Wczytaj więcej aktualności</a>
                    </div>
                </div>
            </div>
        </section>
    ...
    <section id="start" class="bg-navy text-white relative overflow-hidden">
        <div class="relative container-site grid gap-9 items-center" style="grid-template-columns:repeat(auto-fit,minmax(280px,1fr));padding-block:clamp(52px,7vw,80px);">
            <div>
                ...
                <h2 class="font-heading font-extrabold m-0 mb-4" style="font-size:clamp(26px,4vw,42px);line-height:1.1;letter-spacing:-0.01em;">{{ $ctaTytul }}</h2>
                <p class="text-onnavy m-0" style="font-size:16px;line-height:1.65;max-width:560px;">{{ $ctaTekst }}</p>
            </div>
            <div class="flex">
                <a href="{{ $btnUrl($ctaBtnLink) }}" class="btn btn-gold" style="padding:18px 32px;">{{ $ctaBtnLabel }} →</a>
            </div>
        </div>
    </section>
    ...
    {{-- ============ GALERIA ============ --}}
    @if (! empty($galeria))
        <section id="galeria" class="section-y bg-white">
            <div class="container-site">
                <x-section-head
                    eyebrow="Fotorelacje z zawodów"
                    title="Galeria zawodów"
                    :link="$galeriaLink['url'] ?? null"
                    :link-label="$galeriaLink['title'] ?? ($galeriaLink ? 'Wszystkie zdjęcia' : null)" />
                <div class="grid gap-3" style="grid-template-columns:repeat(4,1fr);grid-auto-rows:1fr;">
                    @foreach (array_slice($galeria, 0, 5) as $i => $img)
                        <div class="relative overflow-hidden bg-placeholder {{ $i === 0 ? 'col-span-2 row-span-2' : '' }}" style="aspect-ratio:1/1;">
                            <img src="{{ $img['sizes']['large'] ?? $img['url'] }}" alt="{{ $img['alt'] ?: 'Galeria zawodów mikrolotowych' }}" class="absolute inset-0 w-full h-full object-cover transition-transform duration-300 hover:scale-105" />
                        </div>
                    @endforeach
                </div>
            </div>
