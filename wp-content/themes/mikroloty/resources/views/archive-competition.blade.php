                ...
                <div class="flex gap-1.5">
                    @foreach (['Wszystkie', 'ULM', 'Paralotnia'] as $t)
                        <button type="button"
                            ...
                            :class="filter === '{{ $t }}' ? 'bg-navy text-white border-navy' : 'text-[#3a465f]'"
                ...
                @if (have_posts())
                    <div class="flex flex-col border-t border-line">
                        @while (have_posts()) @php(the_post())
                            ...
                            @endphp
                            <div x-show="filter === 'Wszystkie'  || filter === '{{ $class }}'"
                                ...
                                <div style="flex:1 1 300px;min-width:240px;">
                                    <div class="flex items-center gap-2.5 mb-1.5">
                                        @if ($class)
                                            <span class="uppercase font-bold text-navy" style="font-size:10.5px;letter-spacing:0.06em;background:#eef1f8;border:1px solid #dbe2ef;padding:3px 9px;">{{ $class }}</span>
                                        @endif
